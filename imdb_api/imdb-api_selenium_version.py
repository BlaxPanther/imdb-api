...
class IMDb:
    ...
    def getIdFromSearch(self, search):
        ...
        if source != "":
            if "No results found" in source.find('div',{'class':'sc-17bafbdb-2'}).text  or "Aucun résultat trouvé" in source.find('div',{'class':'sc-17bafbdb-2'}).text :
    ...
    def getPage(url):
        ...
        try:
            ...
            source = BeautifulSoup(resp.replace("&apos;","'").replace("&quot;",'"').replace("&gt;",">").replace( "&lt;","<").replace("&amp;","&"), "html.parser")
            return source
        except:
